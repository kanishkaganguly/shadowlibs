...
/** @brief Get current values of joints from given MoveGroupInterface, they are in the same order
 * as the names of joints obtained from getJointNames()
 *  @param move_group_interface The MoveGroupInterface for the finger
 *  @return The vector of doubles containing the values of joints in that MoveGroup
 */
 ...
 * This is obtained from the /robot_description URDF passed to ROS from the robot
 * @param joint_names The vector of strings containing the names of the joints
 * @return Vector of tuples, each with (upper,lower) limits of each joint, in the same order
 * as the input vector
 */
 ...
 * @return Integer index of BioTac, to be used for fetching correct data from ROS
 */
...
struct Finger {
   ...
   * @brief Vector of ROS publishers, for sending control data for each joint in finger
   */
  std::vector <ros::Publisher> _joint_controller_publishers;

  /**
   * @brief Constructor for each finger. Requires name of finger and a ROS node handle reference.
   * Everything else is populated by constructor.
  ...
  /**
   * @brief Get joint limits from joint name, from vector of tuples populated by getFingerJointLimits()
   ...
   *  @brief Utility struct for managing the BioTac sensor attached to each finger.
   *  Holds the pressure and impedance data obtained from the sensor.
   */
  struct BioTac {
    int16_t pressure;
