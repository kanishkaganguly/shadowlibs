...
namespace shadow_finger {
    ...
    /* Get joint names */
    inline std::vector<std::string> getJointNames(moveit::planning_interface::MoveGroupInterface& move_group_interface) {
    ...
    /* Get joint limits */
    inline int createTreeFromRobotDescription(ros::NodeHandle& n) {
        urdf::Model model;
        ...
        urdf::JointConstSharedPtr j = model.getJoint("rh_FFJ1");
        std::cout << j->name << ": " << j->limits->upper << std::endl;
        std::cout << j->name << ": " << j->limits->lower << std::endl;
    }
