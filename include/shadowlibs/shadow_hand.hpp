 ...
 * 	Constructs "Hand" class of shared pointers to multiple fingers, once
 * multiple finger instances have been created.
 */
...
public:
   ...
   * Shared pointers because you cannot make a Hand without having created
   * Fingers
   */
  std::vector<std::shared_ptr<shadow_finger::Finger>> _fingerVec;

  /**
   * @brief Constructor for Hand class, takes in arbitrary number of shared
   * pointers to Fingers
   * @param fingers_list List of shared pointers to Fingers
   */
  Hand(std::initializer_list<std::shared_ptr<shadow_finger::Finger>>
           fingers_list) {
    ...
    ROS_INFO_STREAM("Initialized Hand with: " << _fingerVec.size()
                                              << " fingers.");
  }
