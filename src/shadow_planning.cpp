...
bool shadow_planning::planToNamedTarget(
    ...
    finger._finger_move_group.setStartState(*finger._finger_move_group.getCurrentState());
    finger._finger_move_group.setPlanningTime(finger._planning_options.set_planning_time);
    finger._finger_move_group.allowReplanning(finger._planning_options.allow_replanning);
    finger._finger_move_group.setNumPlanningAttempts(finger._planning_options.num_attempts);
    finger._finger_move_group.setNamedTarget(target_name);
    bool plan_success = false;

    plan_success = (finger._finger_move_group.plan(finger._plan) == moveit::planning_interface::MoveItErrorCode::SUCCESS);
...
bool shadow_planning::executePlan(
    ...
    for (auto& trajectoryPoint : trajectoryPoints) {
        for (int j = 0; j < jointNames.size(); j++) {
            ...
            control_target.data =
                    trajectoryPoint.positions[jointNames.size() - j];
...
void shadow_planning::executePlanAsync(shadow_finger::Finger& finger) {
    std::future<bool> exec_success = std::async(std::launch::async, [](shadow_finger::Finger& f) { return shadow_planning::executePlan(f); }, std::ref(finger));
    exec_success.get();
}