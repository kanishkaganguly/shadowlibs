...
void shadow_planning::executePlanAsync(shadow_finger::Finger &finger) {
  std::future<bool> exec_success = std::async(std::launch::async, [](shadow_finger::Finger &f) {
    return shadow_planning::executePlan(f);
  }, std::ref(finger));
  exec_success.get();
}

void shadow_planning::executePlanAsync(shadow_hand::Hand &hand) {
  std::vector <std::shared_ptr<shadow_finger::Finger>> fingers = hand.getFingers();
