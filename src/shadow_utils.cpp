...
void shadow_utils::createCollisionObjectFromPrimitive(
	...
	try {
		...
		case shadow_utils::SHAPE_PRIMITIVES::BOX:primitive.type = primitive.BOX;
			primitive.dimensions.resize(3);
			break;
		case shadow_utils::SHAPE_PRIMITIVES::SPHERE:primitive.type = primitive.SPHERE;
			primitive.dimensions.resize(1);
			break;
		case shadow_utils::SHAPE_PRIMITIVES::CYLINDER:primitive.type = primitive.CYLINDER;
			primitive.dimensions.resize(2);
			break;
		case shadow_utils::SHAPE_PRIMITIVES::CONE:primitive.type = primitive.CONE;
			primitive.dimensions.resize(2);
			break;
		}
...
void shadow_utils::startTrajectoryController(ros::NodeHandle n, std::string controller_name) {
	...
	controller_manager_msgs::SwitchController controller_srv;
	std::vector <std::string> controllers_list = {controller_name};
	controller_srv.request.start_controllers = controllers_list;
	controller_srv.request.strictness = controller_manager_msgs::SwitchController::Request::BEST_EFFORT;

	if (client.call(controller_srv)) {
		ROS_INFO("Controller Started: %s", controller_srv.response.ok ? "Yes" : "No");
	} else {
		ROS_ERROR("Failed to call controller manager service");
	}
}
...
void shadow_utils::stopTrajectoryController(ros::NodeHandle n, std::string controller_name) {
	...
	controller_manager_msgs::SwitchController controller_srv;
	std::vector <std::string> controllers_list = {controller_name};
	controller_srv.request.stop_controllers = controllers_list;
	controller_srv.request.strictness = controller_manager_msgs::SwitchController::Request::BEST_EFFORT;

	if (client.call(controller_srv)) {
		ROS_INFO("Controller Stopped: %s", controller_srv.response.ok ? "Yes" : "No");
	} else {
		ROS_ERROR("Failed to call controller manager service");
	}
}