...
void shadow_utils::quat2ypr(tf::Quaternion &q, double &r, double &p,
                            double &y) {
  tf::Matrix3x3 m(q);
  m.getRPY(r, p, y);
}
...
void shadow_utils::removeAllObjectsFromScene(
...
std::vector<ros::Publisher> shadow_utils::createJointControllerPublishers(
...
moveit_msgs::RobotState shadow_utils::computeJointsFromPose(
...
void shadow_utils::startTrajectoryController(ros::NodeHandle n,
  ...
  controller_srv.request.strictness =
      controller_manager_msgs::SwitchController::Request::BEST_EFFORT;

  if (client.call(controller_srv)) {
    ROS_INFO("Controller Started: %s",
             controller_srv.response.ok ? "Yes" : "No");
  } else {
